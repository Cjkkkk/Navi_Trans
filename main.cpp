...
class A {
    ...
    public:
        ...
        size_t do_pack_size() const {
            size_t payload_size = pack_size<INT_32>(generate_key<INT_32>(0));
            ...
            payload_size += pack_size<INT_32>(c);
            return payload_size;
        }
        ...
        size_t do_pack(uint8_t* buf) const {
            ...
            payload_size += pack<INT_32>(buf + payload_size, c);
            return payload_size;
        }
        ...
        size_t do_unpack(uint8_t* buf, size_t& payload_size) {
            ...
            offset += unpack_key<INT_32>(buf + offset, field_number);
            offset += unpack<INT_32>(buf + offset, c);
            return payload_size;
        }
};
...
class B {
    ...
    public:
        ...
        B() {}
        size_t do_pack_size() const {
            size_t payload_size = pack_size<MESSAGE>(a) + pack_size<INT_32>(b) + pack_size<INT_32>(c);
            return payload_size;
        }

        size_t do_pack(uint8_t* buf) const {
            size_t payload_size = pack<MESSAGE>(buf, a);
            payload_size += pack<INT_32>(buf + payload_size, b);
            payload_size += pack<INT_32>(buf + payload_size, c);
            return payload_size;
        }
        ...
        size_t do_unpack(uint8_t* buf, size_t& payload_size) {
            size_t offset = unpack<MESSAGE>(buf, a);
            offset += unpack<INT_32>(buf + offset, b);
            offset += unpack<INT_32>(buf + offset, c);
            return payload_size;
        }
...
class C {
    public:
        ...
        C() {}
        size_t do_pack_size() const {
            size_t payload_size = pack_size<ARRAY>(a) + pack_size<INT_32>(b);
            return payload_size;
        }

        size_t do_pack(uint8_t* buf) const {
            size_t payload_size = pack<ARRAY>(buf, a);
            payload_size += pack<INT_32>(buf + payload_size, b);
            return payload_size;
        }

        size_t do_unpack(uint8_t* buf, size_t& payload_size) {
            size_t offset = unpack<ARRAY>(buf, a);
            offset += unpack<INT_32>(buf + offset, b);
            return payload_size;
        }
...
int main() {
    int32_t data = -300;
    size_t ps = pack_size<INT_32>(data);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf = new uint8_t[ps];
    pack<INT_32>(buf, data);
    unpack<INT_32>(buf, data);
    std::cout << data << "\n";
    delete[] buf;

    int32_t data1 = -320;
    ps = pack_size<SINT_32>(data1);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf1 = new uint8_t[ps];
    pack<SINT_32>(buf1, data1);
    unpack<SINT_32>(buf1, data1);
    std::cout << data1 << "\n";
    delete[] buf1;

    float data2 = 3.2;
    ps = pack_size<FLOAT>(data2);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf2 = new uint8_t[ps];
    pack<FLOAT>(buf2, data2);
    unpack<FLOAT>(buf2, data2);
    std::cout << data2 << "\n";
    delete[] buf2;

    double data3 = 3.4;
    ps = pack_size<DOUBLE>(data3);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf3 = new uint8_t[ps];
    pack<DOUBLE>(buf3, data3);
    unpack<DOUBLE>(buf3, data3);
    std::cout << data3 << "\n";
    delete[] buf3;
    ...
    data4.b = 15;
    data4.c = 20;
    ps = pack_size<MESSAGE>(data4);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf4 = new uint8_t[ps];
    pack<MESSAGE>(buf4, data4);
    // unpack
    unpack<MESSAGE>(buf4, data4);
    std::cout << data4.a << "\n";
    std::cout << data4.b << "\n";
    std::cout << data4.c << "\n";
    delete[] buf4;
    ...
    data5.b = 15;
    data5.c = 20;
    ps = pack_size<MESSAGE>(data5);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf5 = new uint8_t[ps];
    pack<MESSAGE>(buf5, data5);
    // unpack
    unpack<MESSAGE>(buf5, data5);
    std::cout << data5.a.a << "\n";
    std::cout << data5.a.b << "\n";
    std::cout << data5.a.c << "\n";
    std::cout << data5.b << "\n";
    ...
    data6.b = 40;
    ps = pack_size<MESSAGE>(data6);
    std::cout << "pack_size: " << ps << "\n";
    uint8_t* buf6 = new uint8_t[ps];
    pack<MESSAGE>(buf6, data6);
    // unpack
    C data6_1;
    unpack<MESSAGE>(buf6, data6_1);
